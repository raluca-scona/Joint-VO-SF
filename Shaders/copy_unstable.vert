...
void main()
{
    ...
    if(//abs(localNorm.z) > 0.85f &&
       time - vColor.w < timeDelta && localPos.z > 0 && x > 0 && y > 0 && x < cols && y < rows)  //if it has been updated recently and it is a valid point
    {
        for(float i = x / cols - (scale * indexXStep * windowMultiplier); i < x / cols + (scale * indexXStep * windowMultiplier); i += indexXStep)
        {
            for(float j = y / rows - (scale * indexYStep * windowMultiplier); j < y / rows + (scale * indexYStep * windowMultiplier); j += indexYStep)
            {
               ...
               if(current > 0U)
               {
                   ...
                   if(colorTime.z < vColor.z && //if it is an older point
                      vertConf.w > confThreshold &&  //it is a high confidence point
                      vertConf.z > localPos.z &&  //it is behind the current point
                      vertConf.z - localPos.z < 0.01 &&  //but still close enough
                      sqrt(dot(vertConf.xy - localPos.xy, vertConf.xy - localPos.xy)) < vNormRad.w * 1.4)  //if the distance is smaller than the normal radius * 1.4
                   {
                       count++;
                   }
                   
                   if( colorTime.w == time && //just has been updated
                      vertConf.w > 0.5 * confThreshold  &&  //fairly confident */
                      vertConf.z > localPos.z &&  //behind localPos
                      vertConf.z - localPos.z > 0.01) //&&  //further than 10cm
                   {
                       zCount++;
                   }
               }
    ...
    if( (vColor.w == -1 || vColor.y < 0.8 || ((time - vColor.w) > 20 && vPosition.w < confThreshold)) )
    {
        test = 0;
    }
