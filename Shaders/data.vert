...
uniform sampler2D probIsDynamicSamp;
...
void main()
{
    ...
    vec4 probIsDynamicTex = textureLod(probIsDynamicSamp, texcoord.xy, 0); //values are beteween 0 and 1, 1 meaning dynamic

    //Color for color, obviously
    vColor = textureLod(cSampler, texcoord.xy, 0.0);

    float probIsStatic = 1.0 - probIsDynamicTex.x;
    
    vColor.x = encodeColor(vColor.xyz);
    vColor.y = probIsStatic;   //this is the unused one, so safe to modify
    vColor.z = time;
    ...
    //Confidence
    vPosition.w = confidence(x, y, weighting);  //depends on radial distance of point from center of camera
    ...
    if(int(x) % 2 == int(time) % 2 && int(y) % 2 == int(time) % 2 &&  //what does this mean - updating only even or odd points?
