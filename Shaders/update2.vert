...
void main()
{
    ...
    if(newColor.w == -1)
    {
        ...
        vec3 v_g = newPos.xyz;
		  ...
		  float probIsStatic = newColor.y;
		  float stableConfidence = 20.0; //stableThreshold * 5.0;
        ...
        if(newNorm.w < (1.0 + 0.5) * vNormRad.w)
        {
				...
				if (probIsStatic < 0.5 && c_k > stableConfidence) {
					c_k = c_k / 2.0 ;
				}

				float totalConf = c_k + a * probIsStatic;			

				vPosition0 = vec4(((c_k * v_k) + (a * v_g)) / (c_k + a), totalConf);
