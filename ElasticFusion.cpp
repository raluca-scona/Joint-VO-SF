...
void ElasticFusion::createTextures()
{
    ...
    textures[GPUTexture::DEPTH_STATIC_RAW] = new GPUTexture(Resolution::getInstance().width(),
                                                     Resolution::getInstance().height(),
                                                     GL_LUMINANCE16UI_EXT,
                                                     GL_LUMINANCE_INTEGER_EXT,
                                                     GL_UNSIGNED_SHORT);
    ...
    textures[GPUTexture::DEPTH_STATIC_FILTERED] = new GPUTexture(Resolution::getInstance().width(),
                                                          Resolution::getInstance().height(),
                                                          GL_LUMINANCE16UI_EXT,
                                                          GL_LUMINANCE_INTEGER_EXT,
                                                          GL_UNSIGNED_SHORT
    ...
    textures[GPUTexture::DEPTH_STATIC_METRIC] = new GPUTexture(Resolution::getInstance().width(),
                                                        Resolution::getInstance().height(),
                                                        GL_LUMINANCE32F_ARB,
                                                        GL_LUMINANCE,
                                                        GL_FLOAT);

    textures[GPUTexture::DEPTH_STATIC_METRIC_FILTERED] = new GPUTexture(Resolution::getInstance().width(),
                                                                 Resolution::getInstance().height(),
                                                                 GL_LUMINANCE32F_ARB,
                                                                 GL_LUMINANCE,
                                                                 GL_FLOAT,
...
void ElasticFusion::createCompute()
{
    ...
    computePacks[ComputePack::STATIC_FILTER] = new ComputePack(loadProgramFromFile("empty.vert", "depth_bilateral.frag", "quad.geom"),
                                                        textures[GPUTexture::DEPTH_STATIC_FILTERED]->texture);
    ...
    computePacks[ComputePack::STATIC_METRIC] = new ComputePack(loadProgramFromFile("empty.vert", "depth_metric.frag", "quad.geom"),
                                                        textures[GPUTexture::DEPTH_STATIC_METRIC]->texture);

    computePacks[ComputePack::STATIC_METRIC_FILTERED] = new ComputePack(loadProgramFromFile("empty.vert", "depth_metric.frag", "quad.geom"),
                                                                 textures[GPUTexture::DEPTH_STATIC_METRIC_FILTERED]->texture);
}
...
void ElasticFusion::createFeedbackBuffers()
{
    ...
    feedbackBuffers[FeedbackBuffer::STATIC] = new FeedbackBuffer(loadProgramGeomFromFile("vertex_feedback.vert", "vertex_feedback.geom"));
...
void ElasticFusion::computeFeedbackBuffers()
{
    ...
    feedbackBuffers[FeedbackBuffer::STATIC]->compute(textures[GPUTexture::RGB]->texture,   //apply weighted texture onto filtered input cloud instead of rgb
                                                       textures[GPUTexture::DEPTH_STATIC_METRIC_FILTERED]->texture,
                                                       tick,
                                                       maxDepthProcessed);
...
bool ElasticFusion::denseEnough(const Img<Eigen::Matrix<unsigned char, 3, 1>> & img)
{
    ...
    return float(sum) / float(img.rows * img.cols) > 0.4f;
}
                                 ...
                                 const bool bootstrap)
{
    ...
    unsigned short * staticDepth = new unsigned short [ Resolution::getInstance().width() * Resolution::getInstance().height() * 2];
    memcpy ( staticDepth, depth, Resolution::getInstance().width() * Resolution::getInstance().height() * 2);
    ...
    textures[GPUTexture::DEPTH_STATIC_RAW]->texture->Upload(staticDepth, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_SHORT);
    textures[GPUTexture::WEIGHT]->texture->Upload(weightedImageTexture.data(), GL_RGB, GL_UNSIGNED_BYTE);
    ...
    //First run //weighted pyramid should be 1's here...
    if(tick == 1)
    {
        computeFeedbackBuffers();

        globalModel.initialise(*feedbackBuffers[FeedbackBuffer::RAW], *feedbackBuffers[FeedbackBuffer::FILTERED]);   //there should be some weighting here too...
...
void ElasticFusion::predict()
{
    ...
    TICK("FillIn"); //additional data from current image in case the prediction is not dense enough -> this is the case when we have not explored a new area or at the start or so
...
void ElasticFusion::metriciseDepth()
{
    ...
    computePacks[ComputePack::STATIC_METRIC]->compute(textures[GPUTexture::DEPTH_STATIC_RAW]->texture, &uniforms);
    computePacks[ComputePack::STATIC_METRIC_FILTERED]->compute(textures[GPUTexture::DEPTH_STATIC_FILTERED]->texture, &uniforms);
}
...
void ElasticFusion::filterDepth()
{
    ...
    computePacks[ComputePack::STATIC_FILTER]->compute(textures[GPUTexture::DEPTH_STATIC_RAW]->texture, &uniforms);
}
...
void ElasticFusion::getCurrentImages(cv::Mat & colourImage, cv::Mat & depthImage) {
    depthImage = cv::Mat(Resolution::getInstance().height(), Resolution::getInstance().width(), CV_32FC1, 0.0);
    ...
    textures[GPUTexture::DEPTH_METRIC]->texture->Download(depthImage.data, GL_LUMINANCE, GL_FLOAT);
}

void ElasticFusion::getPredictedImages(cv::Mat & colourImage, cv::Mat & depthImage, cv::Mat & weightPrediction) {
    depthImage = cv::Mat( Resolution::getInstance().height(), Resolution::getInstance().width(), CV_32FC1, 0.0);
    colourImage = cv::Mat(Resolution::getInstance().height(), Resolution::getInstance().width(), CV_8UC3,  cv::Scalar(0,0,0));
    weightPrediction = cv::Mat( Resolution::getInstance().height(), Resolution::getInstance().width(), CV_32FC1, 0.0);
    ...
    if (shouldFillIn) {
        std::cout<<"   USING FILLED IN IMAGE\n";

        //fillIn.imageTexture.texture->Download(colourImage.data, GL_RGB, GL_UNSIGNED_BYTE);

        textures[GPUTexture::RGB]->texture->Download(colourImage.data, GL_RGB, GL_UNSIGNED_BYTE);

        cudaArray * textPtr;

        DeviceArray<float> predictedVertexMap;

        predictedVertexMap.create(Resolution::getInstance().height() * Resolution::getInstance().width() * 4);

        cudaGraphicsMapResources(1, &fillIn.vertexTexture.cudaRes);
        cudaGraphicsSubResourceGetMappedArray(&textPtr, fillIn.vertexTexture.cudaRes, 0, 0);
        cudaMemcpyFromArray(predictedVertexMap.ptr(), textPtr, 0, 0, predictedVertexMap.sizeBytes(), cudaMemcpyDeviceToDevice);
        cudaGraphicsUnmapResources(1, &fillIn.vertexTexture.cudaRes);

        float vertexValues[Resolution::getInstance().height() * Resolution::getInstance().width() * 4];

        predictedVertexMap.download(&vertexValues[0]);

        for (int row=0; row<Resolution::getInstance().height();  row++) {
            for (int col=0; col < Resolution::getInstance().width(); col++) {
                depthImage.at<float>(row, col) = vertexValues[row * Resolution::getInstance().width() * 4 + (col * 4) + 2];
            }
        }

    } else {
        std::cout<<"   USING PREDICTED IMAGE\n";


        //indexMap.imageTex()->texture->Download(colourImage.data, GL_RGB, GL_UNSIGNED_BYTE);
        ...
        cudaGraphicsMapResources(1, &indexMap.vertexTex()->cudaRes);
        cudaGraphicsSubResourceGetMappedArray(&textPtr, indexMap.vertexTex()->cudaRes, 0, 0);
        cudaMemcpyFromArray(predictedVertexMap.ptr(), textPtr, 0, 0, predictedVertexMap.sizeBytes(), cudaMemcpyDeviceToDevice);
        cudaGraphicsUnmapResources(1, &indexMap.vertexTex()->cudaRes);
